...
class Segmentation(nn.Module):
    def __init__(self, resnet, in_channels_list=[64, 128, 256, 512], out_channels=32):
        ...
        self.l2 = nn.Linear(1000, 22)

        # variables that are useful to store in the model
        self.resnet_cnn = None
    ...
    def resnet_tail(self, input):
        ...
        """
        output = self.resnet.maxpool(self.resnet.relu(self.resnet.bn1(self.resnet.conv1(input))))
        return output
    ...
    def saved_classification_logits(self):
        # already calculated resnet model output up to layer 4
        output = self.resnet.avgpool(self.resnet_cnn).squeeze(-1).squeeze(-1)
    ...
    def classification_logits(self, input):
